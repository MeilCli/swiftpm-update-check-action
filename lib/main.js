...
function executeOutdated(executeDirectory) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const dependencies = json_1.toPackage(stdout).denpendencies;
        const result = [];
        for (const dependency of dependencies) {
            const directory = path.dirname(path.relative(process.cwd(), dependency.path));
            const tagFile = "cocoapods-update-check.txt";
            yield exec.exec(`git tag > ${tagFile}`, undefined, { cwd: directory });
            const tags = fs
                .readFileSync(path.join(directory, tagFile))
                .toString();
            yield io.rmRF(path.join(directory, tagFile));
