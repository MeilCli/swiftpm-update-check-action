...
import { OutdatedPackage, Package, Dependency, extractJsonStringFromPostfix, toPackage } from "./json";
...
async function executeOutdated(executeDirectory: string | null): Promise<OutdatedPackage[]> {
    ...
    const projectPackage: Package = toPackage(extractJsonStringFromPostfix(stdout));
